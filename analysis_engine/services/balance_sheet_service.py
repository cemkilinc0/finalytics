...
from analysis_engine.tasks.agent_tasks import generate_balance_sheet_analysis
...
class BalanceSheetAnalysisService:
    ...
    def handle(symbol: str) -> list[dict]:
        ...
        return BalanceSheetAnalysisService._process_symbol_query(symbol)
    ...
    def _process_symbol_query(symbol: str) -> list[dict]:
        ...
        try:
            try:
                analysis = BalanceSheetAnalysisModel.objects.get(company__ticker=symbol)
                return [{"message": "Data found in database.", "analysis_data": analysis.analysis}]
            except ObjectDoesNotExist:
                lock_id = f"balance_sheet_analysis_{symbol}"
                ...
                background_task = generate_balance_sheet_analysis.delay(symbol, lock_id, lock.local.token)
