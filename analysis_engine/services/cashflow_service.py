...
from analysis_engine.tasks.agent_tasks import generate_cash_flow_analysis
...
class CashFlowAnalysisService:
    ...
    def handle(symbol: str) -> list[dict]:
        ...
        return CashFlowAnalysisService._process_symbol_query(symbol)

    @staticmethod
    def _process_symbol_query(symbol: str) -> list[dict]:
        logger.info("Received AI analysis request for cash flow statements with symbol", symbol=symbol)
        try:
            try:
                analysis = CashflowStatementAnalysisModel.objects.get(company__ticker=symbol)
                return [{"message": "Data found in database.", "analysis_data": analysis.analysis}]
            except ObjectDoesNotExist:
                lock_id = f"cash_flow_analysis_{symbol}"
                ...
                background_task = generate_cash_flow_analysis.delay(symbol, lock_id, lock.local.token)
